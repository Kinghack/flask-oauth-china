...
class OAuthRemoteApp(object):
    ...
    def authorize(self, callback=None):
        ...
        if self.request_token_url:
            ...
            #secret = session[self.name + '_oauthtok'][1]
            ...
            if callback == None:
                ...
                #url = '%s?oauth_token=%s&oauth_callback=http://peanut:6677%s' % (self.expand_url(self.authorize_url),
            ...
            #if self.name == "renren":
            #    temp = callback
            #    #callback = "http://peanut:6677%s" % temp
            #if self.name == "qq":
            #    temp = callback
    ...
    def handle_oauth2_response(self):
        ...
        if self.access_token_method == 'POST':
            ...
            if self.name == "qq":
                ...
                access_token = content[begin+1:end]
    ...
    def authorized_handler(self, f):
        ...
        def decorated(*args, **kwargs):
            if self.name == 'douban':
                client = self.make_client()
                #head = {'oauth_consumer_key':session[self.name + '_oauthtok'][0]}
                resp, content = client.request('%s' % (     \
                    self.expand_url(self.access_token_url)), \
                    #self.access_token_method, headers=head)
                    self.access_token_method)
                data = parse_response(resp, content)
                if resp['status'] != '200':
                    raise OAuthException('Invalid response from ' + self.name, data)
            elif 'oauth_verifier' in request.args:
